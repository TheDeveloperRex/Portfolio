...
from turtle import bgcolor  # for Python3
...
class AnimatedGif(tk.Label):
	...
	def _animate(self, sub ):
		try:
			self.gif = tk.PhotoImage(file=self.gif_file, format='gif -index {}'.format(self._num) ).subsample( sub, sub )  # Looping through the frames
		...
		if not self.stop:    # If the stop flag is set, we don't repeat
			self.root.after(int(self.delay*1000), self._animate, sub )
	...
	def _animate_thread(self):
		...
		while self.stop is False:  # Normally this would block mainloop(), but not here, as this runs in separate thread
			try:
				...
				self.gif = tk.PhotoImage(file=self.gif_file, format='gif -index {}'.format(self._num), bgcolor=None)  # Looping through the frames
				self.configure(image=self.gif)
				self._num += 1
			except tk.TclError:  # When we try a frame that doesn't exist, we know we have to start over from zero
				self._num = 0
